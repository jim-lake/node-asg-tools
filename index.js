  ...
  async.series(
    [
      ...
      (done) => {
        ...
        const all_id_list = auto_scale_group.Instances.map((i) => i.InstanceId);
      ...
      (done) => {
        ...
        autoscaling.describeAutoScalingGroups(opts, (err, data) => {
          if (err) {
            ...
            const asg_list = data.AutoScalingGroups;
            if (g_config.asg_name) {
              found_asg = _.find(
                asg_list,
                (asg) => asg.AutoScalingGroupName === g_config.asg_name
              );
            } else {
              _.every(asg_list, (asg) => {
                const found_instance = _.find(
                  asg.Instances,
                  (i) => i.InstanceId === instance_id
                );
                if (found_instance) {
                  found_asg = asg;
                }
